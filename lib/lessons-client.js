...
function getAllLocalLessonStatuses() {
	...
	try {
		...
		console.error('Error reading all lesson statuses from localStorage:', error)
		return []
	}
}
...
export function useMarkLessonAsStudied(isUserLoggedIn) {
	...
	return useMutation({
		...
		onMutate: async (lessonId) => {
			...
			const previousAllStatuses = queryClient.getQueryData(lessonKeys.allStatuses())

			queryClient.setQueryData(lessonKeys.allStatuses(), (old = []) => {
