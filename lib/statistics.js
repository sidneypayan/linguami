...
async function awardGoalBonus(supabase, userId, goalType, goalId) {
	try {
		...
		if (!rewardConfig || !rewardConfig.gold_amount) {
			logger.error(`Reward config not found for ${actionType}`)
			return
		}
		...
		logger.log(`🎉 ${goalType} bonus awarded: +${rewardConfig.gold_amount} Gold`)
	} catch (error) {
		logger.error(`Error awarding ${goalType} goal bonus:`, error)
	}
}
