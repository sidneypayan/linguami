 ...
 *
 * Note: Uses coursesClient (PROD DB) for all data (metadata + content)
 */
export async function getLessonData(levelSlug, lessonSlug, learningLang) {
	...
	try {
		// Get course with all lessons from PROD DB
		...
		// Sort lessons for navigation and find the requested lesson
		const sortedLessons = (course.course_lessons || [])
			.filter(l => l.is_published)
			.sort((a, b) => a.order_index - b.order_index)

		const lesson = sortedLessons.find(l => l.slug === lessonSlug)

		if (!lesson) {
			logger.error('Lesson not found:', lessonSlug)
			return null
		}
		...
		return {
			...
			lesson: {
				...lesson,
				// Ensure arrays exist (even if null in DB)
		...
		logger.error('Error in getLessonData:', error)
		return null
	}
}
