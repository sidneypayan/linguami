...
export async function getLeaderboardData(userId) {
	...
	try {
		...
		try {
			...
			logger.warn('Failed to get week bounds:', weekError)
		}
		...
		if (weekStart) {
			...
			if (weeklyError) {
				logger.warn('Weekly leaderboard fetch failed:', weeklyError)
			} else {
				topWeekly = data || []
			}
		}
		...
		try {
			...
			logger.warn('Failed to get month bounds:', monthError)
		}
		...
		if (monthStart) {
			...
			if (monthlyError) {
				logger.warn('Monthly leaderboard fetch failed:', monthlyError)
			} else {
				topMonthly = data || []
			}
		}
