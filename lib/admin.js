...
export async function checkAdminAuth() {
	...
	if (error || !user) {
		// For guests, read language preferences from cookies
		const learningLanguageCookie = cookieStore.get('learning_language')
		const spokenLanguageCookie = cookieStore.get('spoken_language')

		return {
			isAuthenticated: false,
			isAdmin: false,
			user: null,
			learningLanguage: learningLanguageCookie?.value || null,
			spokenLanguage: spokenLanguageCookie?.value || null,
		}
	}
