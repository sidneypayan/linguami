...
async function checkExercises() {
	console.log('Checking exercises for French beautiful-places materials...\n')

	// Get all French beautiful-places materials
	...
	console.log(`Found ${materials.length} French beautiful-places materials\n`)
	...
	// Return list of materials without exercises
	const materialsWithoutExercises = materials
		.filter(m => !allExercises.some(ex => ex.material_id === m.id))
		.map(m => m.id)

	if (materialsWithoutExercises.length > 0) {
		console.log('\nMaterials without exercises:', materialsWithoutExercises.join(', '))
	}
}
